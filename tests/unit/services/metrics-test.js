...
module('Unit | Service | metrics', (hooks) => {
	setupTest(hooks);
	...
	test('#activateAdapters is idempotent', function(assert) {
		...
		assert.equal(get(service, '_adapters.Mixpanel.config.token'),
			'0f76c037-4d76-4fce-8a0f-a9a8f89d1453', 'it does not override the Mixpanel adapter');
	...
	test('#invoke does not leak options between calls', function(assert) {
		...
		assert.ok(GoogleAnalyticsSpy.calledWith({ userName: 'Jimbo', page: 'page/1', title: 'page one', callTwo: true }),
